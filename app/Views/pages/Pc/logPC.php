...
document.addEventListener("DOMContentLoaded", function() {
        ...
        try {
            ...
            if (result.status === 'success') {
                if (result.data && result.data.length > 0) {
                    ...
                    tbody.innerHTML =
                        `<tr><td colspan="5" class="text-center text-danger">ไม่พบข้อมูล</td></tr>`;
                    currentPage = 1;
                    totalPages = 0;
                    totalRows = 0;
                }
                ...
                tbody.innerHTML =
                    `<tr><td colspan="5" class="text-center text-danger">เกิดข้อผิดพลาด</td></tr>`;
            }
        } catch (err) {
            spinner.style.display = "none";
    ...
    document.getElementById("nextPage").addEventListener("click", () => {
        if (currentPage < totalPages) fetchLogPCs({
            page: currentPage + 1,
            keyword: searchInput.value
        });
    });
    ...
    searchInput.addEventListener('input', () => fetchLogPCs({
        page: 1,
        keyword: searchInput.value
    }));
